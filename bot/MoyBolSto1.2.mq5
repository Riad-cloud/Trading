...
void start(){
   ...
   if (MoyMobile!=-1 && LowBB && UpperBB!=-1 && K!=-1 ){//Si il n y a pas eu de problème lors de l'initialisation
      ...
      if(MoyMobile < ask && LowBB >= ask && K <=20 ){//Condition d'achat
         ...
         double StopBuy= 7;//Stoploss
         
         double volumeBuy = CalculatorPst(StopBuy,_Symbol);//Volume
         
         double stoploss = ask - 60*_Point;

         if(trade.Buy(volumeBuy,_Symbol,ask,stoploss,MidBB)){//On Achète
      ...
      //Vente
         ...
         double sellstoploss = bid + 60*_Point;

         if(trade.Sell(volumeSell,_Symbol,bid,sellstoploss,MidBB)){//On Vend
